...
export default class MailerService {
  async sendValidationEmail(user: Users) {
    const databaseMailDataMounted = await this.mountDatabaseMail(
      user,
      EmailsTypesEnum.emailValidation,
    );
    const mailDatabase = await Mails.save(databaseMailDataMounted);
    const emailToSendMounted = await this.mountEmailValidation(
      user,
      mailDatabase,
    );
